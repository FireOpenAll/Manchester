...
public class HotsController {
	...
	public Object getHotsActivities() {
		...
		for (int i = 0; i < 4; i++) {
			interest_list.add(new CategoryModel(10000000+i, "兴趣"+i, "/interest/cover/" + (10 + i) + ".jpg", "兴趣介绍"));
		}
		
		for (int i = 0; i < 10; i++) {
			//recomment_activity.add(new ActivityModel("activity", (long)(11110000+i), "活动名"+i*2, price, like, like_count, join, join_count, comment_count, owner, start_time, end_time, contact, summary, url, photo_list, locationInfo, relative_user, interest_list))
